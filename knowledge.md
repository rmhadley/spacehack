...
The game runtime and packaging are Pygame-only. Keep input, framebuffer, and
...
``docs/design/complete/pytest-coverage.md``.
...
1. **Repetition (DRY)** — Scan the changed file(s) for duplicated patterns. Are there loops, formulas, imports, or entity-construction blocks that appear more than once? Do NOT accept ``it was already duplicated`` as enough — flag it even if it is pre-existing. Extract shared helpers before moving on. Examples of recent misses:
...
3. **Dead code —** Did this change leave unused imports, functions, variables, fields, or constants? Check for functions that were only called from now-replaced code. Remove them.
...
7. **ctx-first** — Is new cross-cutting state going through `GameContext` or being passed around as bare parameters?

8. **Data-first** — Does new content belong in a `data/` catalog + frozen dataclass?

9. **Live-by-side-effect** — Are domain functions mutating state directly or returning values for the caller to apply?

10. **File size** — Check if any touched module exceeds the architecture limits (~1000 lines per module, 40 per function). If so, plan the extraction before committing — the architecture ratchet in ``make check`` enforces these, so do the extraction yourself rather than fighting the gate at commit time.
