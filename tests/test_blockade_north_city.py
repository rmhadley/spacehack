"""Regression coverage for Blockade North's authored Picket garrison."""
...
def test_blockade_north_is_the_authored_picket():
    game_map = load_planet("blockade")
    assert game_map.city_layout_id == "blockade_north_picket"
    assert (game_map.width, game_map.height) == (100, 70)
    assert len(game_map.landmark_stamps) == 3
    # The Corridor spans the deck.
    strip = sum(
        tile.kind == "road" for row in game_map.tiles for tile in row
    )
    assert strip > 100
    # Pressure bulkheads texture the deck.
    assert any(tile.kind == "station_bulkhead" for row in game_map.tiles for tile in row)
    # The command plaza carries the station beacon.
    assert game_map.tiles[34][49].kind == "beacon"
    # Teal/amber/red lighting.
    assert any(tile.kind == "neon" for row in game_map.tiles for tile in row)


def test_blockade_north_buildings_transit_and_npcs_are_reachable():
    game_map = load_planet("blockade")
    spec = find_planet_spec("blockade")
    reachable = _reachable(game_map, spec.hangar_anchor)
    assert set(game_map.city_transit) == {"spaceport", "plaza", "militia", "bounties"}
...
def test_blockade_north_landing_apron_is_smooth_and_showroom_is_clear():
    game_map = load_planet("blockade")
    spec = find_planet_spec("blockade")
...
def test_blockade_north_interiors_follow_authored_conventions():
    game_map = load_planet("blockade")
    assert set(game_map.city_buildings) == {"spaceport", "militia", "bounties"}
...
def test_blockade_north_seeds_military_lighting():
    game_map = load_planet("blockade")
...
def test_blockade_north_has_no_voids_or_unreachable_walkable_cells():
    game_map = load_planet("blockade")
    spec = find_planet_spec("blockade")
