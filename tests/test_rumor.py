...
def test_fire_trigger_hears_the_authored_entry(monkeypatch):
    ...
    assert run(rumor.fire_trigger(ctx, "dock_dark_port")) == ["dark_berth_1"]
    ...
    assert run(rumor.fire_trigger(ctx, "dock_dark_port")) == []
    assert len(_seen) == 1
...
def test_one_event_two_tiers_the_dark_hail(monkeypatch):
    ...
    ctx = SimpleNamespace(known_rumors=[], context=None)
    assert run(rumor.fire_trigger(ctx, "dark_hail")) == ["dark_berth_1"]
    ...
    assert run(rumor.fire_trigger(ctx_mid, "dark_hail")) == []
    ...
    assert run(rumor.fire_trigger(ctx_late, "dark_hail")) == ["dark_berth_3"]


def test_unknown_event_hears_nothing(monkeypatch):
    _presented(monkeypatch, [])
    ctx = SimpleNamespace(known_rumors=[], context=None)
    assert run(rumor.fire_trigger(ctx, "no_such_event")) == []
