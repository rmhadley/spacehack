...
def test_barnards_c_interior_spawns_sit_inside_the_door():
    ...
    mid-floor: the P spawn sits directly inside the exit marker, and the
    exit sits on the door-side south perimeter — the shared
    ``door_placement_error`` rule, which sanctions both the in-wall
    doorway (exit in the wall row, as earth's spaceport authors it) and
    the just-inside row. Regression: both skimmer-deck interiors
    spawned the player in the middle of the room, which no doorway
    works like."""
    game_map = load_planet("barnards_c")
    for label, record in game_map.city_buildings.items():
        asset = city_landmarks.load_city_interior(record["interior_layout_id"])
        ...
        spawn = asset.spawn
        violation = city_landmarks.door_placement_error(
            exits, (spawn.x, spawn.y), interior.height,
        )
        assert violation is None, f"{label}: {violation}"
        ex, ey = exits[0]
