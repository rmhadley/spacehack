...
def load_game(context: "tcod.context.Context") -> GameContext | None:
    ...
    # Ground stats: backward-compatible default of 30/30/30 (0-100 scale,
    # matching the ship-skill base of 30).
    _gsd = _data.get("ground_stats", {}) or {}
    from .character import GroundStats
    _ctx.ground_stats = GroundStats(
        reflexes=_gsd.get("reflexes", 30),
        strength=_gsd.get("strength", 30),
        stamina=_gsd.get("stamina", 30),
    )
