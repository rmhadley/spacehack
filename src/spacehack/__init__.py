...
# first ``import tcod.*`` (verified empirically with SDL_GetHint). Setting the
# hint here, at package-init time and before any tcod import, is what makes
# LINEAR scaling actually take effect — the fix for fractional-DPI displays
# where NEAREST drops glyph pixels. A user shell export works for exactly the
# same reason.
