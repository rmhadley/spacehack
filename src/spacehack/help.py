...
_GUIDE_CONTROLS = GuideSection(
    ...
    body=(
        ...
        "\n"
        "- G: go to a discovered target (stairs, the exit, consoles)"
        "\n\n"
        "Combat keys:"
        "\n"
...
_GUIDE_COMBAT = GuideSection(
    ...
    body=(
        ...
        "Floating numbers:"
        "\n"
        "- Hits float the total damage dealt in orange over the target"
        "\n"
        "- EMP hits float the stripped shields in cyan"
        "\n"
        "- Glancing hits float 'GLANCE -X' in pale gold (half damage)"
        "\n"
        "- Misses float 'MISS' in grey"
        "\n\n"
        "Weapon burst fire:"
        "\n"
        ...
        "Weapon types:"
        "\n"
        "- Energy (lasers): cost power, unlimited ammo; fire an instant"
        "bright beam along the whole line"
        "\n"
        "- Plasma: heavy power cost; fires a slow glowing bolt that travels"
        "cell by cell with a fading trail"
        "\n"
        "- Missile: no power cost, long range, scarce ammo (takes cargo"
        "space). Spent rounds persist until rebought at the mechanic."
        "Missiles fly a slow wobbling rocket with an exhaust trail that"
        "bursts on impact - and overshoots the target on a miss"
        "\n"
        "- EMP Missile: 0 hull damage but strips 20 shields per hit"
        "(ignore armor/hull entirely)"
        "\n\n"
        "Enemies animate their weapons the same way you do - a pirate"
        "firing a laser shows a beam, one firing a missile shows the"
        "rocket flight, and so on."
    ),
)
...
_GUIDE_SHIPS = GuideSection(
    ...
    body=(
        ...
        "\n"
        "- Installed weapons and modules move to global Storage when you"
        "upgrade; they are not destroyed or included in the trade-in value"
        "\n"
        "- Your new ship keeps its catalog starting loadout; install the old"
        "parts later if the new hull has compatible slots"
        "\n"
        "- Mission cargo reservations carry over to the new ship; a warning"
        "appears if the new hull cannot hold the reservation"
        "\n"
        ...
        "\n"
        "- STORE view: left panel lists weapons and modules"
        "(sorted by price); ENTER opens a small chooser with Install "
        "and Store"
        "\n"
        ...
        "opens a small chooser with Install and Sell for X$"
        "\n"
        "- Selling an installed part pays a 50% refund through the My Ship"
        "Store/Sell chooser"
        "\n"
        "- Press B to highlight Buy and show STORE; press S to highlight "
        "Storage and show STORAGE"
        "\n"
        "- Buying does not require an open slot: choose Install to fit "
        "the part now, or Store to keep it for a later ship"
        "\n\n"
        "Ship equipment storage:"
        "\n"
        "- The loadout window has two views: STORE and STORAGE"
        "\n"
        "- Press B to highlight Buy and show STORE; press S to highlight "
        "Storage and show STORAGE; in STORE, ENTER opens the Install/Store "
        "chooser for the selected weapon or module"
        "\n"
        "- STORAGE lists equipment you own but are not using; ENTER opens "
        "a small chooser with Install and Sell for X$"
        "\n"
        "- Install leaves the part in storage if no compatible slot is available"
        "\n"
        "- Stored equipment is global, unlimited, and available at every mechanic"
        "\n"
        "- Stored missile launchers show and retain their remaining ammunition"
        "\n"
        "- Buying a new ship automatically moves every installed part from the"
        "old ship into Storage, including each missile launcher's remaining ammo"
        "\n"
        "- A full or incompatible ship leaves the stored part untouched and logs "
...
_GUIDE_GROUND_GEAR = GuideSection(
    ...
    body=(
        ...
        "two-handed weapon; the second slot shows --- and cannot be selected"
        "\n"
        ...
        "\n\n"
        "Armory equipment ownership:"
        "\n"
        "- Press B to show Buy, A to show Armory Storage, or E to show the"
        "Expedition Pack in the armory terminal; its tab shows current/max"
        " reserve slots (for example, Expedition (2/4))"
        "\n"
        "- Armory Storage is an unlimited global warehouse for owned gear"
        "\n"
        "- The Expedition Pack is a limited reserve: 4 base slots plus"
        "one slot per 10 Strength above 10; equipped items do not consume"
        "pack slots"
        "\n"
        "- Buying opens an Equip, Armory Storage, or Expedition Pack chooser"
        "before credits are deducted"
        "\n"
        "- Stored weapons and armor can be equipped or sold from the armory"
        "with the compact action chooser; displaced gear goes to the Expedition"
        "Pack first, then Armory Storage if the pack is full"
        "\n"
        "- The Expedition Pack is carried into dungeons; press C to open the "
        "Character Equipment tab and view pack contents (current/max)"
        "\n"
        "- Press ENTER on an equipped slot or compatible empty slot to choose a "
        "replacement from the Expedition Pack; outside combat swaps are free, "
        "while successful combat swaps cost 1 AP and close the menu"
        "\n"
        "- Select a BACKPACK ITEMS row and press ENTER for Equip or Discard; "
        "discarding removes the item and gives no credits"
        "\n"
        "- Ground-equipment loot goes into the Expedition Pack, not ship cargo; "
        "if the pack is full, choose one carried item to drop for the new loot, "
        "or leave the new loot on the floor"
        "\n\n"
        ...
        "\n"
        "- In a dungeon or ground combat, the Equipment tab also shows "
        "BACKPACK ITEMS and current/max Expedition Pack capacity"
        "\n"
        "- ENTER on a compatible equipped or empty slot opens backpack swap choices"
        "\n"
        "- Swaps outside combat are free; each successful ground-combat swap costs 1 AP"
        "\n"
        "- Stored gear is separate from the active combat loadout"
    ),
)
...
_GUIDE_GROUND_COMBAT = GuideSection(
    ...
    body=(
        ...
        "\n"        "- Hits float the damage dealt over the target; misses float 'MISS'"
        "\n"
        "\n"
        ...
        "\n"
        "- They fire their equipped weapon when in range, and their attacks"
        "are animated like yours: laser beams, kinetic tracers, plasma bolts,"
        "or melee slashes depending on their weapon, with a floating hit or"
        "MISS number on you"
        "\n"
...
_GUIDE_AUTO_EXPLORE = GuideSection(
    ...
    body=(
        ...
        "of supplies, a quest NPC), a hostile comes into view (LOS "
        "aggro starts the fight), or you press any key to cancel. "
        "Content already in view when you press O does not stop the "
        "run, so you can explore past loot you already spotted."
        "\n\n"
        "A monster you cannot see never blocks auto-explore: the walk "
        "heads into the dark and reveals it when it comes into view. "
        "A hostile you CAN see camping the only exit stops the run "
        "with a \"blocks the only way forward\" message so you know "
        "what stands between you and the unexplored map."
        "\n\n"
        "Go to (G):"
        "\n"
        "- Press G to open a picker of destinations you have discovered "
        "(stairs up/down, the exit, ship computers, quest consoles and "
        "doors, NPCs), nearest first."
        "\n"
        "- The walk uses the same rules as auto-explore: it stops at "
        "newly-visible interesting content, when combat starts, or on "
        "any keypress, and ends beside the target (step onto the stairs "
        "or bump the console yourself)."
        "\n"
        "- A destination you cannot reach logs \"Cannot reach\" and "
        "the walk stops."
        "\n\n"
        "- O: start auto-explore in dungeons (any key cancels)"
        "\n"
        "- G: go to a discovered target (any key cancels)"
    ),
)
