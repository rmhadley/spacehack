...
\"mercury_station\"`). It reads as a compact scorched research base —
the same machinery Earth uses (authored exterior stamps, roof labels,
skyline, sidewalks) but themed for a desert station: a bare heat-shield
deck, a service-road strip, a small commons plaza, and scorched scrub
instead of parks and a river.
...
MERCURY_CITY_WIDTH = 40
MERCURY_CITY_HEIGHT = 30
...
LANDMARK_ORIGINS: dict[str, world.Position] = {
    ...
    "mercury_lab": world.Position(22, 8),
    "mercury_bar": world.Position(4, 20),
    "mercury_supply": world.Position(21, 20),
}

# The deck's service-road strip: two east-west service roads, one under
# the port/lab row and one under the bar/supply row. Every building door
# opens directly onto a road or the pad, so doors never need sidewalk
# bridges. The open deck between the roads stays bare plating for the
# skyline domes to fill.
_ROAD_A_Y = 19
_ROAD_C_Y = 27

# Small commons plaza under the transit hub (a 5x3 patch of deck).
_PLAZA_X_LO, _PLAZA_X_HI = 16, 20
_PLAZA_Y_LO, _PLAZA_Y_HI = 20, 22

# Landing apron below the spaceport (derived from hangar_anchor + port).
_PAD_X_LO, _PAD_Y_LO = 4, 11
...
def _paint_deck_roads(tiles, theme) -> None:
    ...
    for x in range(3, MERCURY_CITY_WIDTH - 2):
        ...
        tiles[_ROAD_C_Y][x] = theme.road_ew
...
def _paint_deck_pad(tiles, theme) -> None:
    """Paint the landing apron below the spaceport."""
    for py in range(_PAD_Y_LO, 16):
        for px in range(_PAD_X_LO, 11):
...
def _new_mercury_map() -> world.GameMap:
...
def _add_service_entities(game_map, spec, resolve_ship) -> None:
    ...
    for ship_id, off_x, off_y in spec.showroom_ships:
        ...
        game_map.entities.append(world.Entity(
            ...
            pos=world.Position(_PAD_X_LO + off_x, _PAD_Y_LO + off_y),
...
def build_mercury_layout(spec, resolve_ship) -> world.GameMap:
    """Build Mercury's 40x30 station deck from data + authored assets.
