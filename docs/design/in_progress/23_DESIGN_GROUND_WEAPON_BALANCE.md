...
> **Status:** Implemented; awaiting manual playtest.
...
- [ ] Fire a Rocket Launcher at one enemy: primary takes full damage and one
      rocket is consumed from the loaded magazine.
- [ ] Fire at two adjacent enemies: the selected target takes full damage and
      the neighbor takes half damage, including odd values rounded down to a
      minimum of 1.
- [ ] Kill only a neighboring enemy with splash: it is removed, awards XP, and
      drops normal loot; the primary remains engaged.
- [ ] Stand adjacent to the impact cell and fire: the player takes half damage.
- [ ] Equip armor and repeat friendly fire: player damage reflects armor DR.
- [ ] Confirm a missed explosive shot causes no splash damage and still consumes
      the shot/AP according to existing firing behavior.
- [ ] Confirm one rocket/grenade is used for the whole blast, not one per target.
- [ ] Compare a Rocket Launcher against a Plasma Caster over several fights:
      rockets feel like a scarce group-control decision, while plasma remains
      a dependable single-target fallback.
- [ ] Verify Grenade Launcher range and Rocket Launcher minimum range still
      matter, including the existing emergency point-blank penalty.
- [ ] Verify dual-wielding the revised one-handed weapons does not produce an
      unexpectedly dominant one-AP volley.
- [ ] Save/Continue after firing and reloading explosives; magazines and reserve
      stacks retain exact values.

## Completion gate

Move this document to `docs/design/complete/` after the manual checklist passes,
the result is recorded, and `make check` remains green.
