...
3. **Minimum wait, never a deadline:** the gate only *unlocks* the next step — the player may answer the summon days or weeks later with zero penalty. Nothing expires, no fail state. The quest log breadcrumb reads "Awaiting word from the <faction>..." while a gate is pending.
